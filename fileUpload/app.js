...
const UPLOAD_DIR = path.resolve(__dirname, ".", "chunkSpace"); // 读取根目录，创建一个文件夹chunkSpace存放切片
...
server.on("request", async (req, res) => {
  ...
  if (req.url === "/upload") {
    ...
    multipart.parse(req, async (err, fields, files) => {
      ...
      const chunkDir = path.resolve(UPLOAD_DIR, `${fileName}-chunks`); //在chunkSpace文件夹创建一个新的文件夹，存放接收到的所有切片
